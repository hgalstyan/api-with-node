const tweetButton = document.getElementById('postTweet');  
const input = document.getElementById('tweet-textarea');
const tweetList = document.getElementById('tweetList');
// function that dynamically creates a markup to encapsulate every new tweet
function buildTweet(tweet, user) {

    let newTweet = document.createElement('li');

    let tweetMarkup = `<strong class="app--tweet--timestamp">now</strong>` +
    `<a class="app--tweet--author">` +
    `<div class="app--avatar" style="background-image: url(${user.profile_image_url});">` +
    `<img src="${user.profile_image_url}">` +
    `</div><h4>${user.name}</h4> @${user.screen_name}</a>` +
    `<p>${tweet}</p>` +
    ...
    newTweet.innerHTML = tweetMarkup;
    return newTweet;
}
// click handler on the 'Tweet' button
tweetButton.addEventListener('click', function (event) {
    event.preventDefault();
    let tweet = input.value;
    window.setTimeout(() => input.value = '', 2000);  // clears the tweet after 2s
    // POST ajax request for a new tweet 
    ...
    xhr.onreadystatechange = function () {
        if (xhr.status === 200 && xhr.readyState === 4) {
            ...
            let newTweet = buildTweet(res.tweetText, res.userInfo);
            tweetList.insertAdjacentElement('afterbegin', newTweet);
        } 
    }
    xhr.send(JSON.stringify({ newTweet: tweet }));
});